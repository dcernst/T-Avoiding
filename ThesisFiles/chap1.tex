...
This thesis is organized as follows. After necessary background information is presented in Section~\ref{sec:coxeter}, we then introduce the class of fully commutative elements in Section~\ref{sec:FC}. Then, in Sectio~\ref{sec:heaps} we discuss a visual representation for elements of Coxeter groups, called heaps. In Section~\ref{sec:star}, we introduce the concept of a star reducible Coxeter group and in Section~\ref{sec:Tavoid} we introduce the notion of a T-avoiding element which is exactly an element that does not begin or end with a product of noncommuting generators. In Section~\ref{sec:noncancel} we state the classification of elements that are non-cancellable in Coxeter systems of type $\C_n$. We then state classifications and conjectures regarding T-avoiding elements in several Coxeter systems in Chapter~\ref{chap:TandTavoid}. Following this we introduce the main results for this thesis, the classification of T-avoiding elements in Coxeter systems of type $B_n$ and $\C_n$. In Section~\ref{sec:Btools} we introduce the necessary lemmas and definitions for the classification in Section~\ref{sec:TAB} in which we show there are no non-trivial T-avoiding elements in Coxeter systems of type $B_n$. In Section~\ref{sec:TAC} we classify the non-trivial T-avoiding elements in Coxeter systems of type $\C_n$. We conclude with some open questions in Section~\ref{sec:open}.
...
\end{subfigure}\label{fig:notFC2}
\begin{subfigure}[b]{0.3\textwidth}	
\centering
\begin{tikzpicture}[scale=0.4]
