...
\begin{proof}
	...
	Therefore, it follow that $W(\C_n)$ does not contain any not FC type II T-avoiding elements. 
\end{proof}
\end{theorem}

We will now consider the elements in $W(\C_n)$ that are FC where we first will classify type II T-avoiding elements in $W(\C_n)$ for $n$ odd and then proceed to the classification for $n$ even.

\begin{theorem}
	If $n$ is odd, then there are no type II T-avoiding elements in the Coxeter system of type $\C_n$.
	\begin{proof}
		Consider the Coxeter system of type $\C_n$. By Theorem~\ref{thm:TavoidC} we know that $W(\C_n)$ contains no type II T-avoiding elements that are not FC. Recall $W(\C_n)$ is a star reducible Coxeter group, which implies that $W(\C_n)$ contains no type II T-avoiding elements that are FC. Thus as $W(\C_n)$ has no type II T-avoiding elements that are FC or not FC, $W(\C_n)$ has no type II T-avoiding elements.
	\end{proof}
\end{theorem}
...
\begin{theorem}
	...
	\begin{proof}
		Let $w \in W(\C_n)$. By Theorem~\ref{thm:TavoidC}, we know that $w$ is an FC element. Further, we can restrict our search down to type II non-cancellable elements. \textcolor{green}{Specifically we can consider the non-cancellable elements that do not contain Property T}. In Remark~\ref{rem:noncancel} we stated the classification of the only type II T-avoiding elements with full support. Recall these to be sandwich stacks. Thus the only type II T-avoiding elements in $W(\C_n)$ for $n$ odd are sandwich stacks.
	\end{proof}
\end{theorem}
