...
There are no non-trivial T-avoiding elements in $W(\C_n)\setminus \FC(\C_n)$. 	
\begin{proof}
...
\begin{theorem}
	...
	\begin{proof}
		Let $w \in W(\C_n)$. By Theorem~\ref{thm:TavoidC}, we know that $w$ is an FC element. Further, we can restrict our search down to non-trivial non-cancellable elements. \textcolor{green}{Specifically we can consider the non-cancellable elements that do not contain Property T}. In Remark~\ref{rem:noncancel} we stated the classification of the only non-trivial T-avoiding elements with full support. Recall these to be sandwich stacks. Thus the only non-trivial T-avoiding elements in $W(\C_n)$ for $n$ odd are sandwich stacks.
	\end{proof}
\end{theorem}
