...
We will now classify the type II T-avoiding elements in $W(\C_n)$. We first classify type II T-avoiding elements in $W(\C_n)$ for $n$ odd and then proceed to the classification for $n$ even.

\begin{theorem}
	If $n$ is odd, then there are no type II T-avoiding elements in the Coxeter system of type $\C_n$.
	\begin{proof}
		Consider the Coxeter system of type $\C_n$. By Theorem~\ref{thm:TavoidC} we know that $W(\C_n)$ contains no type II T-avoiding elements that are not FC. Recall $W(\C_n)$ is a star reducible Coxeter group, which implies that $W(\C_n)$ contains no type II T-avoiding elements that are FC. Thus as $W(\C_n)$ has no type II T-avoiding elements that are FC and no type II T-avoiding elements that are not FC, $W(\C_n)$ has no type II T-avoiding elements.
	\end{proof}
\end{theorem}
