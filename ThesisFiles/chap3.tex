...
\begin{proof}
	Let $w=uv$ (reduced) is a Coxeter system of type $\C_n$ such that $v$ is FC and has a reduced expression beginning in $s_0s_1 \cdots s_{n-1}s_ns_{n-1}$. Then $w$ has the convex subheap seen in Figure~\ref{fig:lemzigzag}. Since $v$ is FC, this is the only Notice that if $v$ is just the subheap seen in Figure~\ref{fig:lemzigzag}, then $w$ has Property T on the right. Notice that in order for the subword to remain FC the only possibility for a new block to be added to the bottom of the subheap is the block for $s_{n-2}$. Again if this were the bottom of the subheap $w$ would have Property T on the right. Iterating this process we see that we can only create a heap with a single diagonal zig-zag that changes direction when it reaches $s_0$ or $s_n$. This implies that 
	\[
...
\begin{proof}
	...
	Case (1): In this case, we are assuming that the braid is located in consecutive rows with the upper-most block in either row 3 or row 4 and the lowest block involved in the braid is located in the bottom-most row of the heap.
	
	Subcase (1.1): Suppose $k=3$, this implies $m(s_{i-1}, s_i)=3$. Without loss of generality assume $s_{i-1}$ is in the bottom-most row of the heap. Clearly, the block for $s_{i+1}$ must be in the bottom-most row of the heap as well, otherwise $w$ has Property T, which is a contradiction to the original choice of $w$. Restricting our focus to the subheap of $w$ that contains the braid we are considering,  we see that the subheap of $w$ has the following form
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
	...
	\end{tikzpicture}	
	\end{center}
 	here the blocks for $s_{i-1}$ and $s_{i+1}$ are in the bottom-most row of the heap and are thus fully exposed. Applying the braid move we get the subheap seen here
 	\begin{center}
 	\begin{tikzpicture}[scale=0.45]
	...
	Subcase (1.2): Suppose $k=4$, this implies $m(s_{i-1}, s_i)=4$. Without loss of generality assume $s_{i-1}=s_0$ and $s_{i}=s_1$. 
	
	Subcase (1.2.1): First we take $s_0$ to be the topmost brick in the heap. The we obtain the subheap seen here
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
	...
	\end{tikzpicture}	
	\end{center}
	which clearly has Property T on the bottom. This is a contradiction to the way in which we chose $w$.
	
	Subcase (1.2.2): Now we take $s_1$ to be the topmost brick in the heap. Clearly, the block for $s_2$ must be in the bottom-most row of the heap as well, otherwise $w$ has Property T, which is a contradiction to the orginal choice of $w$. Restricting our focus to the subheap of $w$ that contains the braid we are considering, we see that the subheap of $w$ has the following form
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
	...
	\end{tikzpicture}
	\end{center}
	where we have highlighted the braid in \textcolor{teal}{teal}. Notice that the block for $s_{i-2}$ is present in the $(k-2)$th row otherwise there is a braid with lower height and $w$ would not be reduced. Applying the braid move to the heap we get the following subheap
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
	...
	\end{tikzpicture}
	\end{center}
	which has a new braid in it. This braid, which we have highlighted in \textcolor{rred}{red} for emphasis, has height $k-1$. In applying the braid move we have obtained a heap which has a braid with lower height than our original choice. This is a contradiction to the way in which we chose our heap.
	...
	Case (3): Suppose the braid has height $k$ and assume the braid contains $s_2$ or $s_{n-2}$. This implies that either $s_{i-1}=s_1$ or $s_i=s_{n-1}$. Without loss of generality we assume that the braid contains $s_2$ as the other argument is symmetric to the one presented here, and assume $s_2$ is in the $k$th row. Notice that if the braid contains $s_2s_3s_2$, we are in Case (2), as a result we assume our braid is of the form $s_1s_2s_1$ or $s_2s_1s_2$. 
	
	Subcase (3.1): Let the block for $s_1$ is in row $k$, then as we are not in Case (1) we know that at least one of the blocks for $s_0, s_2$ are in the $(k-3)th$ row. 
	...
	\end{tikzpicture}	
	\end{center}
	where we have highlighted the braid in \textcolor{teal}{teal} and have indicated with dotted blocks positions which cannot have heap blocks. Applying the braid move to the heap we get the following subheap
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{3}{2}{}{black}
		\heapblock{0}{0}{0}{purple}
	...
	\end{tikzpicture}	
	\end{center}
	Notice that the new braid now has height $k-1$. This contradicts the way in which we chose the heap of $w$. 
	
	Subcase (3.1.2): Assume that the block for $s_2$ is in row $k$. Potentially the block for $s_0$ is in row $k$, however it does not have to be we emphasize this in the following heap with a \textcolor{purple}{purple}-striped block.  Then the subheap we are considering is as follows
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{4}{}{black}
		\heapblock{2}{0}{2}{purple}
	...
	\end{tikzpicture}	
	\end{center}
	where the braid we are consider in the heap is highlighted in \textcolor{teal}{teal}. Notice that if this was the heap for $w$, $w$ would not be reduced which forces the block for $s_3$ to be in the heap which is seen here.
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{4}{}{black}
		\heapblock{2}{0}{2}{purple}
	...
	\end{tikzpicture}	
	\end{center}
	where a new braid has appeared which we have highlighted in \textcolor{rred}{red}. Notice that the height of this new braid is $k-1$. This braid is lower in the heap than our original braid which is a contradiction to the original choice of $w$.
	
	Subcase (3.2): Assume $s_2$ is in row $k$. This implies that at least one of the blocks for $s_1$ or $s_3$ is in the $(k-3th)$ row. We proceed in cases.
	...
	\end{tikzpicture}	
	\end{center}
	here the braid is now has height $k-1$. The new braid now has height less than $k$ which is a contradiction to the way in which we chose the heap for $w$.
	...
	\end{tikzpicture}	
	\end{center}
	Notice that there are no new braids. However, we know that this can not be the bottom row of our subheap as then $w$ has Property T on the right. This implies that the block for $s_2$ is in the row beneath the blocks for $s_1$ and $s_3$. Notice that $s_0$ cannot be in this row since otherwise the heap would contain a lower braid, this leads to the heap 
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{-2}{}{black}
		\heapblock{1}{0}{1}{purple}
	...
	\end{tikzpicture}	
	\end{center}
	here we have emphasized that $s_0$ cannot be in the position with a dotted block. Again this cannot be the bottom row of the subheap and utilizing the same argument we get the following heap
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{-2}{}{black}
	...
	\end{tikzpicture}	
	\end{center}
	Notice that this can not be the bottom row of the heap. Iterating this argument we get the following heap
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{4}{}{black}
		\dheapblock{1}{2}{}{black}
	...
	\end{tikzpicture}	
	\end{center}
	Notice that this heap has the pattern seen in Lemma~\ref{lem:zigzag} so $w$ has Property T on the right. This is a contradiction to the way in which we chose $w$.
	
	Subcase (3.2.3): Assume that both blocks for $s_1$ and $s_3$ are in row $k-3$. Then the sub heap we are considering is
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\heapblock{1}{0}{1}{purple}
		\heapblock{3}{0}{3}{purple}
	...
	\end{tikzpicture}	
	\end{center}
	where we have highlighted the additions in \textcolor{rred}{red} blocks. We have also placed a dotted block in the place where $s_0$ and $s_4$ would be to signify that no block may appear here. Notice that if this new row is row 1, then $w$ would have Property T with respect to $s_2$ and $s_1$. Again, this implies that this will not be the bottom row of the heap. Repeating this process again we see that having $s_1$ or $s_3$ will create a braid in the $(k-5)$th row. Then we have the subheap 
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{0}{}{black}
		\heapblock{2}{0}{2}{rred}
	...
	\end{tikzpicture}	
	\end{center}
	where we have highlighted the additions in \textcolor{rred}{red} blocks. We have also placed a dotted block in the place where $s_0$ and $s_2$. Notice that if this new row is row 1, then $w$ would have Property T with respect to $s_3$ and $s_4$. Again, this implies that this will not be the bottom row of the heap. Repeating this process again we obtain the subheap
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\sheapblock{3}{-2}{3}{rred}
	...
	\end{tikzpicture}	
	\end{center}
	where we have highlighted the additions in \textcolor{rred}{red}-striped blocks. Notice that if the block for $s_3$ is in that position we have a  braid in a lower position. So assume the block for $s_3$ is not in that position. Then we have the subheap
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{3}{-2}{}{black}
	...
	\end{tikzpicture}
	\end{center}
	where we have highlighted the additions in \textcolor{rred}{red} blocks and have placed a dotted block in the position for $s_3$ indicating that no block may appear there. Iterating this process we end up with the subheap 
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{3}{-2}{}{black}
		...
		\dheapblock{11}{-8.5}{}{black}
		\dheapblock{10}{-10.5}{}{black}
		\dheapblock{6}{-10.5}{}{black}
		\dheapblock{7}{-8.5}{}{black}
		\dheapblock{6}{-6.5}{}{black}
		\heapblock{8}{-6.5}{n-1}{rred}
		\heapblock{9}{-8.5}{n}{rred}
		\heapblock{8}{-10.5}{n-1}{rred}
	\end{tikzpicture}
	\end{center}
	...
	\end{tikzpicture}	
	\end{center}
	where we have highlighted the additions in \textcolor{rred}{red} blocks and have placed dotted blocks in the position for $s_0$ in that row, as if it were to appear the heap would have a lower braid. This heap has Property T in the bottom with respect to $s_2$ and $s_1$. Thus this can not be the bottom row of our heap. Repeating this process, we see that $s_1$ will not be in row $k-5$ since this would create a lower braid. Thus we must have $s_3$ or $s_5$ in the $(k-5)$th row. We represent this with the following subheap 
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{2}{}{black}
		\dheapblock{1}{0}{}{black}
	...
	\end{tikzpicture}	
	\end{center}
	where again we have highlighted the additions in \textcolor{rred}{red} blocks and put dotted blocks where no block appears. Notice that if we were to only place one of the blocks for $s_3$ or $s_5$ we would quickly be in Subcase (ii) or (iii) above. Thus we know we must place both $s_3$ and $s_5$ in the $(k-5)th$ row. Again, if the $(k-5)$th row is the first row, then  $w$ would have Property T with respect to $s_3$ and $s_2$. This implies that the $(k-5)$th row is not the bottom-most row in our heap. Iterating this process we obtain the following subheap
	\begin{center}
	\begin{tikzpicture}[scale=0.45] 
		\dheapblock{0}{2}{}{black}
		\dheapblock{1}{0}{}{black}
...
%	where we see that this has led to $s_0$ being located in the $(k-4)$th row. We emphasize here with the not labeled \textcolor{rred}{red}-striped blocks that every possible brick is present in the area that was outlined in the previous heap. Notice that a new braid has now appeared in the heap. We have highlighted this in \textcolor{orange}{orange} above. This new braid has height $k-1$. This is a contradiction as the braid appears lower than the original braid we chose. 
	...
	Subcase (4.1): Assume the block for $s_2$ is located in the $(k-3)$th row. Then the subheap we are considering is
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{2}{4}{}{black}
	...
	\end{tikzpicture}
	\end{center}
	in which we see that the original braid is now located higher in the heap with height $k+1$. Since $k > 4$ (otherwise in Case (1)), we know that in the original heap, $s_0$ and $s_2$ are located above row 1. This implies that the subheap has more rows underneath which we will now systematically fill in. 
	...
	\end{tikzpicture}	
	\end{center}
	where a new braid appears  which we have highlighted in \textcolor{rred}{red}. Notice that the block for $s_0$ cannot be in between the blocks for $s_1$ which are highlighted in \textcolor{rred}{red}, since otherwise $w$ was not reduced. This is seen in the heap here
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\dheapblock{0}{2}{}{black}
		\dheapblock{2}{6}{}{black}
	...
	\end{tikzpicture}	
	\end{center}
	where we have indicated the position of the block for $s_0$ with a black dotted block. This new braid has height $k-2$ which is lower than the height of the original braid that we chose. This is a contradiction to the way in which we chose $w$. 
	
	Subcase (4.1.2): Now we consider if the block for $s_3$ is in the $(k-4)$th row and the block for $s_1$ is not. This leads to the subheap seen here 
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		%\dheapblock{3}{4}{}{black}
		\dheapblock{1}{0}{}{black}
		%\dheapblock{4}{2}{}{black}
		\heapblock{3}{0}{3}{rred}
	...
	\end{tikzpicture}
	\end{center}
	where again there are no new braids present. Although there are no new braids present, the bottom row of the subheap is not the bottom row of the heap of $w$ since otherwise $w$ would have Property T. Repeating the above argument we extend our heap to look like
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
	...
	\end{tikzpicture}	
	\end{center}
	where the zig-zag pattern continues. Then by Lemma~\ref{lem:zigzag} we know that the heap has Property T on the bottom. This is a contradiction to the way in which we chose the heap for $w$. 
	
	Case (5): Suppose the braid has height $k$ and assume the braid contains $s_0$ or $s_n$. Without loss of generality we assume that the braid contains $s_0$ as the other argument is symmetric to the one presented here, and assume $s_0$ is in row $k$. We now consider what is in row $k-4$. Notice that $s_0$ cannot be in the $(k-4)$th row as $w$ would not be reduced. This implies that $s_2$ is in the $(k-4)$th row as the $(k-3)th$ row cannot be row 1 otherwise we are in Case (1.2.1). From this we get the subheap
	\begin{center}
	\begin{tikzpicture}[scale=0.45]
		\heapblock{2}{0}{2}{purple}
		\dheapblock{0}{0}{}{black}
