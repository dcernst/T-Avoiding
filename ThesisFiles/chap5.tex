...
In this section we summarize the already known classification of the T-avoiding elements in Coxeter systems of type $D_n$. Recall that $W(D_n)$ is a star reducible Coxeter group and as a result any potential non-trivial T-avoiding elements are not FC.
...
In this section we state the already known classification of what is known regarding the T-avoiding elements in the Coxeter systems of type $F_n$ for $n \geq 4$. Note that all of the following results are unpublished. %Recall that $W(F_n)$ is a star reducible Coxeter group so any non-trivial T-avoiding element in $W(F_n)$ will not be FC.
